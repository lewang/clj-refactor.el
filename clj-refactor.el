...
point is assumed to be at the function name"
  (unless (cljr--no-changes-to-parameter-order? signature-changes)
    (let ((num-partials 0)
      ...
      ;; TODO At present this only handles situations where the final
      ;; parameters are swapped e.g.:
      ;; (map (partial my-fn 1) 2 3) and args 2 and 3 swapped
      ;; There's no reason we can't support every use-case though.
      (if (<=  min-index num-partials)
          (cljr--append-to-manual-intervention-buffer)
        (goto-char end)
        (cljr--update-call-site signature-changes)))))
